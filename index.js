...
app.post("/", async (req, res) => {
  ...
  logger.info(new Date() + "------"  + "Full Request body : " + JSON.stringify(req.body));

  //Defining a variable to catch and use the request body
  let receivedData = req.body.creq;
  logger.info(new Date() + "------"  + "Recieved Encoded CReq : " + JSON.stringify(receivedData ));
  ...
  try {
    ...
    logger.info(new Date() + "------"  + "Recieved Encoded CReq decoded to String : " + JSON.stringify(decodedBuffer));

    //Parsing the CReq
    decodedBuffer = JSON.parse(decodedBuffer);
    logger.info(new Date() + "------"  + "Recieved Encoded CReq String Parsed and converted to JSON : " + JSON.stringify(decodedBuffer));
    ...
    logger.info(new Date() + "------"  + "New item added to DB : " + JSON.stringify(dataBase[dataBase.length-1]));
    ...
    if (
      ...
      logger.error(new Date() + "------"  + "Entered CReq is not valid, costumer Redirected to ACS Page");
      res.sendFile(__dirname + "/public/error.html");
    } else {
      //Else costumer redirected to Error page
      logger.info(new Date() + "------"  + "Entered CReq is valid, Costumer Redirected to ACS Page");
      res.sendFile(__dirname + "/public/webpage.html");
    }
    /**********************************/
  } catch (error) {
    //If there is an error to Parsing data , costumer goes to Error page
    logger.error(new Date() + "------"  + "Error occured while parsing the CReq, costumer Redirected to ACS Page. Error Message : " + JSON.stringify(error.message));
    res.sendFile(__dirname + "/public/error.html");
  }
  ...
  logger.info("");
  logger.info("");
  databaseDirectory.info("database after update '/' : " + JSON.stringify(dataBase));
...
app.get("/getId", (req, res) => {
  ...
  logger.info(new Date() + "------"  + "Response to GET request : " + JSON.stringify(dataBase[dataBase.length - 1]));
  logger.info("---------------------------------------------------------------------------------------------------");
  logger.info("");
  logger.info("");
  logger.info("");
...
app.post("/final", async (req, res) => {
  ...
  logger.info(new Date() + "------"  + "Defining this ID's CReq to a variable for Generating CRes");
  ...
  logger.info(new Date() + "------"  + "New CRes generated : " + JSON.stringify(dataBase[req.body.id].cres));
  ...
  await fetch("http://10.10.40.33:8080/directory-server/check/otp", {
    ...
    .then((res) => res.text())
    .then((restext) => {
      logger.info(new Date() + "------"  + "Request has been sent to Backend, Result : Successful");
  ...
  res.send("ok");
  logger.info(new Date() + "------"  + "Response has been sent to Frontend : " + "ok");
  logger.info("---------------------------------------------------------------------------------------------------");
  logger.info("");
  logger.info("");
  logger.info("");
  ...
  databaseDirectory.info("database after update '/final' : " + JSON.stringify(dataBase));
});
...
app.post("/unsuccess", async (req, res) => {
  ...
  logger.info("");
  logger.info("");
  databaseDirectory.info("database after update '/unsuccess' : " + JSON.stringify(dataBase));
